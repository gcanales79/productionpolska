    ...
    function ProduccionTurnoDia() {
        ...
        $.when(
            ...
            for (let i = 0; i < datosDia.length; i++) {
                ...
                ProduccionTurnoDia[indice] = parseInt(datosDia[i].stf4_hr10)+parseInt(datosDia[i].stf4_br10)+parseInt(datosDia[i].stf4_hr16)
            }
            let ProduccionTurnoTarde = [0, 0, 0, 0, 0, 0, 0];
            for (let i = 0; i < datosTarde.length; i++) {
                var indice = datosTarde[i].dia;
                //!Este dato se cambia dependiendo de la linea
                ProduccionTurnoTarde[indice] = parseInt(datosTarde[i].stf4_hr10)+parseInt(datosTarde[i].stf4_br10)+parseInt(datosTarde[i].stf4_hr16)
            }
            let ProduccionTurnoNoche = [0, 0, 0, 0, 0, 0, 0];
            for (let i = 0; i < datosNoche.length; i++) {
                var indice = datosNoche[i].dia;
                //!Este dato se cambia dependiendo de la linea
                ProduccionTurnoNoche[indice] = parseInt(datosNoche[i].stf4_hr10)+parseInt(datosNoche[i].stf4_br10)+parseInt(datosNoche[i].stf4_hr16)
            }
            ...
            .then(function (data) {
                ...
                if (data.length === 0) {
                    ...
                    for (let j = 0; j < data.length; j++) {
                        //!Este dato se cambia dependiendo de la linea
                        Reportesemana.push(parseInt(data[j].stf4_hr10)+parseInt(data[j].stf4_br10)+parseInt(data[j].stf4_hr16))
                    }
